...
SELECT
    YEAR(created_date) AS [Năm],
    FORMAT(SUM(business_result), 'N2') AS [Tổng Giá Trị (VNĐ)],
    CAST(SUM(business_result) / 1000000000.0 AS DECIMAL(18, 2)) AS [Tổng Giá Trị (Tỷ VNĐ)],
    COUNT(*) AS [Số Lượng Lead]
FROM lead
WHERE status IN ('Qualified', 'Proposal Sent', 'In Negotiation', 'Won', 'Lost')
